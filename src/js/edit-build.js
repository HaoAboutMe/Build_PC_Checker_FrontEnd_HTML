...
function setupCoreEvents() {
    ...
    document.getElementById("picker-search").oninput = (e) => {
        ...
        filteredItems = pickerItems.filter(item => item.name.toLowerCase().includes(query));
        pickerPage = 1;
        renderPickerPage();
    };

    document.getElementById("page-prev-btn").onclick = () => { if(pickerPage > 1) { pickerPage--; renderPickerPage(); }};
    document.getElementById("page-next-btn").onclick = () => { if(pickerPage < Math.ceil(filteredItems.length / 8)) { pickerPage++; renderPickerPage(); }};
