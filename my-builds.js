...
function getImageUrl(path) {
  ...
  // and needs to be formatted as http://localhost:8080/identity/{path}
...
async function loadMyBuilds() {
    ...
    gridEl.innerHTML = '<div class="empty-state">Đang tải danh sách cấu hình...</div>';
...
async function openBuildDetail(id) {
    ...
    listEl.innerHTML = "<p style='text-align:center; padding: 1rem;'>Đang tải chi tiết...</p>";
    ...
    if (res && res.code === 1000 && res.result) {
        ...
        document.getElementById("modal-build-desc").textContent = build.description || "Không có mô tả.";
        
        const parts = build.parts || {};
        
        const componentIcons = {
            cpu: "🖥️", mainboard: "🛹", ram: "🎛️", vga: "🎮", gpu: "🎮",
            ssd: "💽", hdd: "💿", psu: "🔋", cooler: "❄️", case: "🗄️"
        };

        const categories = {
            cpu: "CPU", mainboard: "Mainboard", ram: "RAM", vga: "VGA", gpu: "VGA",
            ssd: "SSD", hdd: "HDD", psu: "Nguồn", cooler: "Tản nhiệt", case: "Vỏ Case"
        };
        ...
        if (listEl.innerHTML === "") {
            listEl.innerHTML = "<p style='text-align:center; padding: 1rem; color: #999;'>Không có thông tin linh kiện chi tiết.</p>";
        }
    } else {
        listEl.innerHTML = "<p style='text-align:center; padding: 1rem; color: #f56565;'>Lỗi khi tải chi tiết cấu hình.</p>";
    }
}
...
function setupModalEvents() {
    ...
    document.getElementById("build-detail-overlay").addEventListener("click", closeModal);
}
