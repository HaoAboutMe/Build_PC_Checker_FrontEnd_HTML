...
const elements = {
  ...
  closeChangePasswordModal: document.getElementById(
...
function switchAdminTab(tabName) {
  ...
  elements.permissionsTabContent.classList.toggle(
    "active",
    tabName === "permissions",
  );
...
async function loadAllUsers() {
  elements.usersTableBody.innerHTML =
    '<tr><td colspan="6" class="loading-row">Đang tải...</td></tr>';
  ...
  try {
    ...
    if (response.code === 1000) {
      ...
      if (users.length === 0) {
        elements.usersTableBody.innerHTML =
          '<tr><td colspan="6" class="loading-row">Không có người dùng nào</td></tr>';
        return;
      }
                        ...
                        ${
                          user.roles && user.roles.length > 0
                            ? user.roles
                                .map(
                                  (r) =>
                                    `<span class="role-badge" style="font-size: 0.75rem; padding: 0.25rem 0.75rem;">${r.name}</span>`,
                                )
                                .join(" ")
      ...
      elements.usersTableBody.innerHTML =
        '<tr><td colspan="6" class="loading-row">Lỗi tải dữ liệu</td></tr>';
    }
  } catch (error) {
    showToast("Lỗi", error.message, "error");
    elements.usersTableBody.innerHTML =
      '<tr><td colspan="6" class="loading-row">Lỗi kết nối</td></tr>';
  }
}
...
async function loadAllRoles() {
  elements.rolesTableBody.innerHTML =
    '<tr><td colspan="4" class="loading-row">Đang tải...</td></tr>';
  ...
  try {
    ...
    if (response.code === 1000) {
      ...
      if (roles.length === 0) {
        elements.rolesTableBody.innerHTML =
          '<tr><td colspan="4" class="loading-row">Không có vai trò nào</td></tr>';
        return;
      }

      elements.rolesTableBody.innerHTML = roles
        .map(
          (role) => `
                <tr>
      ...
      handleApiError(response);
      elements.rolesTableBody.innerHTML =
        '<tr><td colspan="4" class="loading-row">Lỗi tải dữ liệu</td></tr>';
    }
  } catch (error) {
    showToast("Lỗi", error.message, "error");
    elements.rolesTableBody.innerHTML =
      '<tr><td colspan="4" class="loading-row">Lỗi kết nối</td></tr>';
  }
}
  ...
  try {
    ...
    if (response.code === 1000) {
      ...
      if (permissions.length === 0) {
        elements.permissionsTableBody.innerHTML =
          '<tr><td colspan="3" class="loading-row">Không có quyền hạn nào</td></tr>';
        return;
      }

      elements.permissionsTableBody.innerHTML = permissions
        .map(
          (permission) => `
                <tr>
      ...
      handleApiError(response);
      elements.permissionsTableBody.innerHTML =
        '<tr><td colspan="3" class="loading-row">Lỗi tải dữ liệu</td></tr>';
    }
  } catch (error) {
    showToast("Lỗi", error.message, "error");
    elements.permissionsTableBody.innerHTML =
      '<tr><td colspan="3" class="loading-row">Lỗi kết nối</td></tr>';
  }
}
...
async function openEditUserModal(userId) {
  try {
    ...
    if (response.code === 1000) {
      ...
      const adminCheckbox = document.querySelector(
        '#edit-user-roles input[value="ADMIN"]',
      );
      if (adminCheckbox) {
        const hasAdminRole =
...
async function loadPermissionsCheckboxes() {
  elements.rolePermissions.innerHTML =
    '<p class="loading-text">Đang tải quyền hạn...</p>';
  ...
  try {
    ...
    if (response.code === 1000) {
      ...
      if (permissions.length === 0) {
        elements.rolePermissions.innerHTML =
          '<p class="loading-text">Chưa có quyền hạn nào</p>';
        return;
      }

      elements.rolePermissions.innerHTML = permissions
        .map(
          (permission) => `
      ...
      elements.rolePermissions.innerHTML =
        '<p class="loading-text">Lỗi tải quyền hạn</p>';
    }
  } catch (error) {
    elements.rolePermissions.innerHTML =
      '<p class="loading-text">Lỗi kết nối</p>';
  }
}
    ...
    if (response.code === 1000) {
      ...
      if (userInfoResponse.code === 1000) {
        showToast(
          ...
          `Chào mừng ${userInfoResponse.result.firstname}!`,
          "success",
        );
...
if (elements.closeChangePasswordModal) {
  elements.closeChangePasswordModal.addEventListener(
    "click",
    closeChangePasswordModal,
  );
}
if (elements.changePasswordOverlay) {
  elements.changePasswordOverlay.addEventListener(
    "click",
    closeChangePasswordModal,
  );
}
if (elements.cancelChangePassword) {
  elements.cancelChangePassword.addEventListener(
    "click",
    closeChangePasswordModal,
  );
}
...
if (elements.createPermissionBtn) {
  elements.createPermissionBtn.addEventListener(
    "click",
...
if (elements.permissionModalOverlay) {
  elements.permissionModalOverlay.addEventListener(
    "click",
    closePermissionModal,
  );
}
