...
async function checkToken(token) {
  const response = await fetch("/identity/auth/introspect", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
async function logout() {
  ...
  const response = await fetch("/identity/auth/logout", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
async function getMyInfo() {
  ...
  const response = await fetch("/identity/users/me", {
    method: "GET",
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
async function deleteRole(roleName) {
  ...
  const response = await fetch(`/identity/roles/${roleName}`, {
    method: "DELETE",
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
async function createPermission(permissionData) {
  ...
  const response = await fetch("/identity/permissions", {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
...
async function deletePermission(permissionName) {
  ...
  const response = await fetch(`/identity/permissions/${permissionName}`, {
    method: "DELETE",
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
const loginResult = await api.post(
  ...
  true,
); // skipAuth = true vì đây là public endpoint
...
function validateForm(formData) {
  ...
  if (
    formData.password &&
    formData.password.length < ValidationRules.password.minLength
  ) {
...
async function loadUserInfo() {
  ...
  try {
    ...
    if (response.code === 1000) {
      ...
      document.getElementById("dateOfBirth").textContent = formatDate(
        user.dateOfBirth,
      );
...
function sanitizeInput(input) {
  ...
  return input.trim().replace(/[<>]/g, ""); // Remove < và >
}
```
...
```bash
# Login
curl -X POST /identity/auth/token \
  -H "Content-Type: application/json" \
  -d "{\"email\":\"admin@example.com\",\"password\":\"password123\"}"

# Get users (với token)
curl -X GET /identity/users \
